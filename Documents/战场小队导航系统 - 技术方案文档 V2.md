...
| 组件（Component） | 挂到宿主项目任意 Actor / Character 子类上 |
| 世界子系统（WorldSubsystem） | 自动存在，无需手动创建 |
| 行为树节点（BT Task / Decorator） | 留作参考，宿主项目可直接使用或替换为 StateTree |
| 状态树节点（StateTree Task / Evaluator / Condition） | 提供 5 个任务 + 2 个评估器 + 5 个条件，覆盖最小决策闭环（详见 §X StateTree 决策层） |
| NavArea 类 | 项目中直接引用 |
