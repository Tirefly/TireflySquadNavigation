...
| 状态树节点（StateTree Task） | 预留扩展位，后续补充 |
| NavArea 类 | 项目中直接引用 |
...
void AMyAIController::OnPossess(APawn* InPawn)
{
    ...
    // ★ 冗余安全：TsnTacticalMovementComponent 构造函数已将 bUseRVOAvoidance 置 false，
    //   此处再次显式关闭，防止宿主项目意外在 BP 中将其重新开启，与 DetourCrowd 发生双重避障冲突。
    if (UTsnTacticalMovementComponent* MoveComp =
            Cast<UTsnTacticalMovementComponent>(InPawn->GetMovementComponent()))
    {
